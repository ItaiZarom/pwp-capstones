...
class TextSample:
    def __init__(self, text, author):
        self.text = text
        self.author = author
        self.average_sentence_length = get_average_sentence_length(self.author)
        self.prepared_text = prepare_text(self.text)
        self.word_count_frequency = frequency_table(self.prepared_text)
    def __repr__(self):
        return self.author + " average sentence length: " +str(self.average_sentence_length)
...
def build_frequency_table(corpus):
    frequency = {}
    for word in corpus:
        frequency[word] = corpus.count(word)
    return frequency
...
def ngram_creator(self, text_list):
    pairs = [text_list[i] + ' ' + text_list[i+1] for i in range(len(text_list)-1)]
    print(pairs)
...
def frequency_comparison(table1, table2):
        ...
        for i in table1:
            if i in table2:
                mutual_appearences += min(table1[i], table2[i])
                appearances += max(table1[i], table2[i])
            else:
                appearances += table1[i]
        for i in table2:
            if i not in table1:
                appearances += table2[i]
        return mutual_appearences / appearances
...
def percent_difference(self, textSample1, textSample2):
       return abs(textSample1 - textSample2)/ ((textSample1+textSample2)/2)
...
def find_text_similarity(textSample1, textSample2):
    ...
    word_count_similarity = frequency_comparison(textSample1,textSample2)
